...
class Document{
	...
	public static function get_documents($conn, $view, $user_id){
		...
		if($view=='manager'){
			//Get all documents
			$sql = "SELECT `b`.*, `d`.*, `dt`.`doc_type_name`, `ds`.`doc_status_name`, DATE_FORMAT(DATE_ADD(`date_created`, INTERVAL `dt`.`doc_type_duration` DAY), '%D %M %Y') AS deadline
			...
			WHERE `doc_sender_user_id`='$user_id' AND `doc_sender_user_id`=`user_id` AND `document`.`document_id`=`box`.`document_id` 
				AND `document_status`.`doc_status_id` = `document`.`doc_status_id` AND `doc_type`.`doc_type_id`=`document`.`doc_type_id`";
